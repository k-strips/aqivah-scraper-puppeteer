...
const logActiveStatus = async () => {
  try {
    console.log(
      "*** scraper is currently active. logging status to database ****"
    );
...
const goToPage = async ({
  ...
  if (paginationTypeOptions === "CLICK") {
...
const initializeScrapingSession = async (sourceId) => {
  try {
    ...
    console.log(
      `\n*** ${celebrate} successfully initialized scraping session ${JSON.stringify(
        response
      )}`
    );
    return response.id;
    ...
    return {
      success: false,
      message: "Failed to initialize scraping session",
      error: e,
    };
  }
};
...
const storeError = async ({ error, scrapingSessionId }) => {
  try {
    ...
    const response = await ScrapersApi.storeError({
      error,
      id: scrapingSessionId,
    });
    console.log(
    ...
    return {
      success: false,
      message: "failed to store failure of scraping session",
    };
  }
};
